...
import * as dataFormat from 'pages/CRUD/Medications/table/MedicationsDataFormatters';

// eslint-disable-next-line
import * as usersDataFormat from 'pages/CRUD/Users/table/UsersDataFormatters';
// eslint-disable-next-line
import * as medication_fillsDataFormat from 'pages/CRUD/Medication_fills/table/Medication_fillsDataFormatters';

import actions from 'store/actions/medications/medicationsListActions';
...
const MedicationsTable = () => {
  const [filters, setFilters] = React.useState<any[]>([
    { label: 'Medication Name', title: 'medication_name' },
    { label: 'Quantity', title: 'quantity', number: 'true' },
    { label: 'Days Supply', title: 'days_supply', number: 'true' },
    { label: 'Days Before Refill', title: 'days_before_refill', number: 'true' },

    { label: 'Medication Fills', title: 'medication_fills' },
  ]);
  ...
  const loading = useTypedSelector((store) => store.medications.list.loading);
  const rows = useTypedSelector((store) => store.medications.list.rows);
  const count = useTypedSelector((store) => store.medications.list.count);
  const modalOpen = useTypedSelector((store) => store.medications.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.medications.list.idToDelete);
      ...
      <div>
        {null && (
          <Button
            color="default"
            size="xs"
            onClick={() => dispatch(push(`/admin/medications/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color="info"
          size="xs"
          onClick={() => history.push(`/admin/medications/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  const columns: Array<any> = [
    ...
    {
      dataField: 'days_before_refill',
      sort: true,

      text: 'Days Before Refill',
    },

    {
      dataField: 'medication_fills',
      sort: true,

      formatter: medication_fillsDataFormat.listFormatter,

      text: 'Medication Fills',
    },
  ...
  return (
    <div>
      <Widget title={<h4>Medications</h4>} collapse close>
        <Link to="/admin/medications/new">
