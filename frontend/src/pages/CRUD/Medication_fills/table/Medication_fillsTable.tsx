...
import * as dataFormat from 'pages/CRUD/Medication_fills/table/Medication_fillsDataFormatters';

// eslint-disable-next-line
import * as medicationsDataFormat from 'pages/CRUD/Medications/table/MedicationsDataFormatters';

import actions from 'store/actions/medication_fills/medication_fillsListActions';
...
const Medication_fillsTable = () => {
  ...
  const loading = useTypedSelector((store) => store.medication_fills.list.loading);
  const rows = useTypedSelector((store) => store.medication_fills.list.rows);
  const count = useTypedSelector((store) => store.medication_fills.list.count);
  const modalOpen = useTypedSelector((store) => store.medication_fills.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.medication_fills.list.idToDelete);
      ...
      <div>
        {null && (
          <Button
            color="default"
            size="xs"
            onClick={() => dispatch(push(`/admin/medication_fills/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color="info"
          size="xs"
          onClick={() => history.push(`/admin/medication_fills/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  return (
    <div>
      <Widget title={<h4>Medication_fills</h4>} collapse close>
        <Link to="/admin/medication_fills/new">
...
export default Medication_fillsTable;
